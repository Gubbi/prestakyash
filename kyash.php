...
//		return Db::getInstance()->execute($sql);
    ...
    */
    public function hookPaymentReturn($params)
    {
        ...
        if ($state == Configuration::get('PS_OS_PREPARATION') || $state == Configuration::get('PS_OS_OUTOFSTOCK'))
        {
            ...
            $expires_on = $this->getKyashOrder($params['objOrder']->id,'kyash_expires');
    ...
    public function hookActionOrderStatusPostUpdate($params)
    {
        ...
        if($id_order > 0)
        {
            ...
            $api = $this->getKyashApiInstance(Configuration::get('kyash_public_api_id'),Configuration::get('kyash_api_secret'), Configuration::get('kyash_callback_secret'), Configuration::get('kyash_hmac_secret'));
            $api->setLogger($this);
    ...
    */
    ...
    public function addKyashOrder($id_cart,$id_order,$code,$expires_on)
    {
        $sql = 'INSERT INTO `'._DB_PREFIX_.'kyash_order` (`id_cart`, `id_order`, `kyash_code`, `kyash_status`, `kyash_expires`) VALUES('.(int)($id_cart).','.(int)($id_order).', "'.pSQL($code).'","pending",'.(int)($expires_on).')';
        return Db::getInstance()->execute($sql);
    }
    ...
    public function getKyashApiInstance($key,$secret,$callback_secret,$hmac)
    {
        return new KyashPay($key,$secret,$callback_secret,$hmac);
    }
